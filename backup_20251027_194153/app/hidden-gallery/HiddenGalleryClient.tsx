...
export default function HiddenGalleryClient({ images }: HiddenGalleryClientProps) {
  ...
  const getImageCategory = (imageName: string) => {
    ...
    if (name.includes('sunset') || name.includes('sunrise')) return { icon: '🌅', label: 'Golden Hour', color: 'from-amber-500 to-orange-500' };
    if (name.includes('cave') || name.includes('underground')) return { icon: '🕳️', label: 'Hidden Caves', color: 'from-gray-600 to-gray-800' };
    if (name.includes('water') || name.includes('lake') || name.includes('pond')) return { icon: '💧', label: 'Secret Waters', color: 'from-blue-500 to-teal-500' };
    if (name.includes('temple') || name.includes('heritage')) return { icon: '🏛️', label: 'Ancient Sites', color: 'from-purple-500 to-indigo-500' };
    if (name.includes('wildlife') || name.includes('bird')) return { icon: '🦅', label: 'Wildlife Secrets', color: 'from-green-500 to-emerald-500' };
    return { icon: '🗺️', label: 'Uncharted', color: 'from-emerald-500 to-teal-500' };
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-purple-100">The secret gallery is still being assembled...</p>
        </div>
      </div>
    );
  }
          ...
          {images.map((image, index) => {
            ...
            return (
              ...
              >
                ...
                <button
                  onClick={() => openModal(image, index)}
                  className="absolute inset-0 w-full h-full focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
