...
const KutchMap: React.FC = () => {
  ...
  if (!isMounted || !isLoaded) {
    return (
      <div className="relative w-full h-80 bg-gradient-to-br from-gray-50 to-gray-50 rounded-xl border border-gray-200 overflow-hidden shadow-inner flex items-center justify-center">
        <div className="text-center">
          <div className="text-4xl mb-2">🗺️</div>
          <p className="text-gray-600 font-inter text-sm">Loading interactive map...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full h-80 bg-gradient-to-br from-gray-50 to-gray-50 rounded-xl border border-gray-200 overflow-hidden shadow-inner">
      {/* SVG Map */}
      <svg
        viewBox="0 0 500 350"
      ...
      {/* Info panel */}
      {hoveredDest && (
        <div className="absolute top-4 left-4 bg-white/95 backdrop-blur-sm rounded-lg p-3 shadow-xl border border-gray-200 transition-all duration-200">
          {(() => {
            const dest = destinations.find(d => d.id === hoveredDest);
            return dest ? (
              <div>
                <h5 className="font-inter font-bold text-sm text-gray-700">{dest.name}</h5>
                <p className="text-xs text-gray-100 mt-1">{dest.description}</p>
                <button 
                  onClick={() => handleDestinationClick(dest.slug)}
                  className="text-xs text-gray-600 hover:text-gray-700 mt-1 font-medium transition-colors"
                >
                  Explore →
                </button>
              </div>
      ...
      <div className="absolute bottom-4 right-4 bg-white/95 backdrop-blur-sm rounded-lg p-3 shadow-lg border border-gray-200">
        <h6 className="font-inter font-bold text-xs text-gray-700 mb-2">Map Legend</h6>
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-xs">
            <div className="w-2 h-2 bg-gray-500 rounded-full"></div>
            <span className="text-gray-100">Heritage Sites</span>
          </div>
          <div className="flex items-center gap-2 text-xs">
            <div className="w-2 h-2 bg-gray-500 rounded-full"></div>
            <span className="text-gray-100">Coastal Areas</span>
          </div>
          <div className="flex items-center gap-2 text-xs">
            <div className="w-2 h-2 bg-gray-500 rounded-full"></div>
            <span className="text-gray-100">Desert Regions</span>
          </div>
        </div>
      </div>
    </div>
